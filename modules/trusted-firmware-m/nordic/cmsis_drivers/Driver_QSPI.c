...
static const ARM_FLASH_CAPABILITIES DriverCapabilities = {
	.event_ready = 0, .data_width = DATA_WIDTH_32BIT, .erase_chip = 0};

static ARM_FLASH_INFO FlashInfo = {.sector_info = NULL, /* Uniform sector layout */
				   .sector_count = QSPI_FLASH_TOTAL_SIZE /
...
static const nrfx_qspi_config_t pQSPIConf =
	NRFX_QSPI_DEFAULT_CONFIG(RTE_QSPI0_SCL_PIN, RTE_QSPI0_CSN_PIN, RTE_QSPI0_IO0_PIN,
				 RTE_QSPI0_IO1_PIN, RTE_QSPI0_IO2_PIN, RTE_QSPI0_IO3_PIN);

static bool bQSPIInit;
...
static int32_t ARM_QSPI_Flash_Initialize(ARM_Flash_SignalEvent_t cb_event)
{
	...
	if (bQSPIInit == false) {
		if (nrfx_qspi_init(&pQSPIConf, NULL, NULL) != 0) {
...
static int32_t ARM_QSPI_Flash_ReadData(uint32_t addr, void *data, uint32_t cnt)
{
	...
	if ((addr % QSPI_READ_SIZE_ALIGNMENT) != 0) {
		...
		if (nrfx_qspi_read(baTmpRAMBuffer, QSPI_MIN_READ_SIZE, read_pos) != 0) {
			return ARM_DRIVER_ERROR_PARAMETER;
		}
	...
	while (bytes > 0) {
		...
		if (bytes < read_size) {
			...
			if ((read_size % QSPI_READ_SIZE_ALIGNMENT) != 0) {
				...
				read_size += QSPI_READ_SIZE_ALIGNMENT -
					     (read_size % QSPI_READ_SIZE_ALIGNMENT);
			}
		}

		if (nrfx_qspi_read(baTmpRAMBuffer, read_size, addr) != 0) {
			return ARM_DRIVER_ERROR_PARAMETER;
		}
...
static int32_t ARM_QSPI_Flash_ProgramData(uint32_t addr, const void *data, uint32_t cnt)
{
	uint32_t err;
...
{
	ARM_FLASH_STATUS status = {.busy = (nrfx_qspi_mem_busy_check() == 0)};
