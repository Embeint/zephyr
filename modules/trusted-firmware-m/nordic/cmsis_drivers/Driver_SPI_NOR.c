...
static nrfx_spim_t spi_instance = NRFX_SPIM_INSTANCE(FLASH_SPI_NOR_BUS_REG);
			  ...
			  size_t length)
{
	...
	return (err == 0) ? ARM_DRIVER_OK : ARM_DRIVER_ERROR;
}
...
static int32_t ARM_SPI_NOR_Flash_Initialize(ARM_Flash_SignalEvent_t cb_event)
{
	...
	if (spi_nor_is_inited == false) {
		...
		if (nrfx_spim_init(&spi_instance, &spim_config, NULL, NULL) != 0) {
