...
static bool i2c_nrfx_twim_rtio_start(const struct device *dev)
{
	...
	switch (sqe->op) {
	case RTIO_OP_RX:
		return i2c_nrfx_twim_rtio_msg_start(dev, I2C_MSG_READ | sqe->iodev_flags, sqe->buf,
						    sqe->buf_len, dt_spec->addr);
	case RTIO_OP_TINY_TX:
		return i2c_nrfx_twim_rtio_msg_start(dev, I2C_MSG_WRITE | sqe->iodev_flags,
						    sqe->tiny_buf, sqe->tiny_buf_len,
						    dt_spec->addr);
	case RTIO_OP_TX:
		/* If buffer is not accessible by DMA, copy it into the internal driver buffer */
		if (!nrf_dma_accessible_check(&config->common.twim, sqe->buf)) {
			/* Validate buffer will fit */
			if (sqe->buf_len > config->common.msg_buf_size) {
				LOG_ERR("Need to use the internal driver "
					...
					"\"%s\"' node.",
					sqe->buf_len, config->common.msg_buf_size, dev->name);
				return i2c_rtio_complete(ctx, -EINVAL);
			}
			memcpy(config->common.msg_buf, sqe->buf, sqe->buf_len);
			sqe->buf = config->common.msg_buf;
		}
		return i2c_nrfx_twim_rtio_msg_start(dev, I2C_MSG_WRITE | sqe->iodev_flags, sqe->buf,
						    sqe->buf_len, dt_spec->addr);
