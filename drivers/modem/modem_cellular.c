							...
							enum modem_cellular_event evt)
{
	...
	case MODEM_CELLULAR_EVENT_CMUX_DISCONNECTED:
		 ...
		 */
		disconnect_timeout_ms = config->vendor->cmux_disconnect_timeout_ms > 0
...
int modem_cellular_pm_action(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_SUSPEND:
		if (current_thread == k_work_queue_thread_get(&k_sys_work_q)) {
			 ...
			 * how to avoid it.
			 * This should not happen unless a user is directly using PM operations on
			 * the system workqueue, as the driver requests the dedicated PM workqueue,
			 * which network requests are routed through.
			 */
