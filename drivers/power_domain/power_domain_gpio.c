...
#define _INST_RAW_FOLLOWERS(id)                                                                    \
	DT_INST_FOREACH_PROP_ELEM_SEP(id, raw_follower_gpios, GPIO_DT_SPEC_GET_BY_IDX, (, ))

/* Generate array of raw follower GPIOs */
#define RAW_FOLLOWERS(id, name)                                                                    \
	static const struct gpio_dt_spec name[] = {COND_CODE_1(                                    \
		DT_INST_NODE_HAS_PROP(id, raw_follower_gpios), (_INST_RAW_FOLLOWERS(id)), ())}
