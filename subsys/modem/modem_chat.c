...
static bool modem_chat_send_script_request_part(struct modem_chat *chat)
{
	...
	if (script_chat->request_size == MODEM_CHAT_REQUEST_FN_LEN) {
		request_size = script_chat->request_fn(&request_str, chat->user_data);
	} else {
		request_str = script_chat->request;
		request_size = script_chat->request_size;
	}
	total_size = request_size + chat->delimiter_size;

	if (chat->script_send_pos < request_size) {
		/* Still sending the request */
		buf = &request_str[chat->script_send_pos];
		buf_len = request_size - chat->script_send_pos;
		extra_buf = (uint8_t *)chat->delimiter;
		extra_len = chat->delimiter_size;
	} else {
		/* Purely into the delimiter */
		buf = &chat->delimiter[chat->script_send_pos - request_size];
