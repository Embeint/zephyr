...
ZTEST(net_buf_tests, test_net_buf_1)
{
	...
	for (i = 0; i < bufs_pool.buf_count; i++) {
		zassert_equal(bufs_pool.buf_count - i, net_buf_num_free(&bufs_pool));
	...
	for (i = 0; i < ARRAY_SIZE(bufs); i++) {
		zassert_equal(i, net_buf_num_free(&bufs_pool));
		net_buf_unref(bufs[i]);
	}
	zassert_equal(bufs_pool.buf_count, net_buf_num_free(&bufs_pool));
